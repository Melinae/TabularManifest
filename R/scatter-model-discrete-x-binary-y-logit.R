...
scatter_model_discrete_x_binary_y_logit <- function(
  ...
  g_obs <- ggplot2::ggplot(ds_plot, ggplot2::aes_string(x=xName, y=yName)) +
  ...
  g_predicted <- ggplot2::ggplot(ds_plot, ggplot2::aes_string(x=xName, y=yHatName)) +
  ...
  g_residual <- ggplot2::ggplot(ds_plot, ggplot2::aes_string(x=xName, y=residualName)) +
    ggplot2::geom_point(pch=1, alpha=alphaPoint, na.rm=T, position=jitterPredicted) +
